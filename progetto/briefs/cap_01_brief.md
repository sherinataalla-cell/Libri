---
unita_id: cap_01
titolo: L'eredità trappola
slug: l_eredita_trappola
posizione: 1
registro: teso e asciutto, doppia voce (Francesca analitica / Giovanni materico)
lunghezza_target: 3000
ambient_conditions: {"season": null, "time_of_day": "giorno", "weather": null, "wind": null}
status_brief: bozza
generato_il: 2026-06-06
schema_brief_version: "1.0"
---

# Brief — L'eredità trappola

## 2 — Core narrativo

**Premessa:** Francesca Ferrante, convocata a Bologna per quella che crede una formalità ereditaria, scopre di aver ereditato metà di Palazzo Aldovrandi insieme a un coerede sconosciuto, con una clausola che impone sei mesi di residenza congiunta.

**Problema:** Il coerede è Giovanni Liguori, l'architetto del restauro che vede in lei 'la donna che venderà il palazzo': due volontà opposte (vendere vs restaurare filologicamente) incatenate dalla stessa clausola.

**Soglia:** Il notaio consegna le chiavi dell'unico attico abitabile: da stasera Francesca e Giovanni dovranno dividerlo.

**Risoluzione:** Hook d'azione — la gabbia si chiude (forced proximity installata). Ostilità dichiarata, nessuna risoluzione.

## 3 — Narrazione fattuale (integrale)

# Narrazione fattuale — Cap. 01 · L'eredità trappola
...
> Fonte canonica: `framework_strutturale.md` (CAP 1) + `bibbia.md` §4 (motore del conflitto) + `carta_voce.md` §4 (simbolo ⚖️ ⛓️).

---

## Beat A — Francesca (POV "lei")

- Francesca Ferrante (31, analista per un fondo immobiliare milanese) è convocata a Bologna, allo studio di un notaio, per la successione di una prozia.
- Crede sia una formalità: una firma, la chiusura di una pratica. Arriva con la testa già al portfolio — vede la quota da vendere, la plusvalenza, lo scatto di carriera.
- Il notaio legge il testamento. Emerge la **clausola**: Francesca eredita **metà** di Palazzo Aldovrandi in **comproprietà indivisa** con un coerede a lei sconosciuto.
- La clausola impone **sei mesi di residenza congiunta** nel palazzo. Per sei mesi non si può né vendere, né affittare, né disporre della quota. **Chi si ritira prima perde tutto a favore dell'altro.**
- Reazione da analista: calcola, cerca la via d'uscita, mette a bilancio. Per lei il palazzo è un asset con una plusvalenza dietro l'angolo (ha già in mente il fondo come acquirente).

## Beat B — Giovanni (POV "lui")

- Entra il coerede: è **Giovanni Liguori** (36, architetto, restauro conservativo). È l'architetto che la prozia aveva incaricato del restauro del palazzo.
- Per Giovanni, Palazzo Aldovrandi è l'opera della vita: restauro filologico, niente compromessi.
- Riconosce subito in Francesca "la donna che lo venderà": l'erede con la calcolatrice al posto del cuore.
- **Prima scintilla d'ostilità** reciproca. Due volontà opposte — vendere vs restaurare — incatenate dalla stessa clausola. Nessun contatto fisico ancora: solo sguardi e disprezzo.
...
- `palazzo_aldovrandi` (oggetto del contendere, campo condiviso)
- `bologna` (studio del notaio)
- `milano` (la vita/carriera di Francesca, fuori scena, evocata)
- `fondo_immobiliare` (antagonista esterno, evocato — non in scena)
- `clausola_testamentaria` (oggetto-motore della trama)

## Semi piantati qui

- La clausola dei sei mesi e la regola "chi si ritira perde tutto" (motore di tutto il libro).
- Il conflitto di valori vendere↔restaurare.
- La forced proximity nell'attico (un solo attico, mezza ristrutturazione → prelude a "una sola doccia", cap. 5).
- Il fondo immobiliare come deadline esterna (rilancio al cap. 13).

## Debiti narrativi aperti

- Chi terrà il palazzo alla scadenza dei sei mesi? (si chiude in Atto III)
...
- **Location**: bologna (qualifier: studio del notaio)
- **Cast presente**: francesca_ferrante
- **Focal action**: Il notaio legge la clausola: metà palazzo in comproprietà indivisa, sei mesi di residenza obbligata, un coerede sconosciuto.
- **Atmosfera**: la gabbia si rivela, ⛓️
- **Palette**: stretta, vincolante
...
- **Focal action**: Entra Giovanni, l'architetto del restauro; i due si riconoscono come avversari e scatta la prima scintilla d'ostilità.
- **Atmosfera**: elettrica, ostile (nessun contatto)
- **Palette**: scintilla a freddo
...
- **Location**: bologna (qualifier: studio del notaio)
- **Cast presente**: francesca_ferrante, giovanni_liguori
- **Focal action**: Il notaio consegna le chiavi dell'unico attico abitabile: dovranno dividerlo a partire da stasera.
- **Atmosfera**: trappola che scatta, ⛓️
- **Palette**: vincolo + minaccia di prossimità
...
- **Modalità attiva**: analitica, di controllo (quantifica per non sentire)
...
- **Modalità attiva**: intransigente, materica (parla delle cose per non parlare di sé)
- **Vincoli locali**: {"max_detti": null, "max_onomatopee_firma": null}
...
**Callback chiamati qui**: _(nessuno)_

**Seeds piantati qui**: seed_clausola_sei_mesi, seed_conflitto_vendere_vs_restaurare, seed_forced_proximity_attico, seed_fondo_deadline
**Seeds che fioriscono qui**: _(nessuno)_
**Seeds che maturano qui**: _(nessuno)_

**Debts aperti qui**: debt_chi_tiene_il_palazzo
